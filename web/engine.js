...
export class Life {
  ...
  weightedPick(wanted) {
    ...
    for (let i = 0; i < this.choices.length; i++) {
      const [name, minutes] = this.choices[i];
  ...
  morningOf(full) {
    ...
    this.canDo = this.weightedPick(Math.max(1, energy)).map(
      ([name, minutes]) => ({ name, kind: 0, minutes }));
    if (this.mustDo.length < 9 && this.rint(2) === 0) {
      this.mustDo.push({ name: this.pick(MUST_DO[this.stage]), kind: 1,
                         minutes: this.rrange(20, 90) });
    }
