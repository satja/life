...
const SCHOOLED = new Set(["in the office", "at the school"]);
...
const SPELLS = [
  ...
  ["a long time, with %s", 1,
   [["looking at photographs", 45, "evening"],
    ["telling the story again", 35, "evening"]], null],
];
...
const CAN_STILL_DO = new Set(["how to look busy", "how to sit through things"]);
...
export class Life {
  ...
  compose(age, stage) {
    ...
    for (const who of this.learned || []) take(who, KNOWN);
    return out;
  }
  ...
  goToSchool() {
    ...
    // what school left you that you only know, rather than do
    this.learned = this.skills.filter((s) => !CAN_STILL_DO.has(s)).map((w, i) =>
      ({ key: "k" + i, word: w, kind: "known", since: 18, gone: null,
         estranged: false, at: "in" }));
  }
  ...
  buildSchedule() {
    ...
    add(this.rrange(19, 26) + (this.average >= 2.45 ? 2 : 0), "first job", "work");
    ...
    const when = [];
    for (let k = 0, n = this.rrange(1, 4); k < n; k++) {
      const at = this.rrange(moved + 3, moved + 30);
      if (!when.includes(at)) when.push(at);
    }
    when.sort((a, b) => a - b);
    when.forEach((at, i) => {
      const spell = SPELLS[i % SPELLS.length];
      add(at, spell[0].replace("%s", them), ["spell", i % SPELLS.length, them]);
    });
  ...
  restock() {
    ...
    for (const s of this.skillsKept) if (CAN_STILL_DO.has(s)) choices.push([s, 45]);
  ...
  keepTheBooks(age) {
    ...
    if (age >= 18 && age < 66 && this.hadJob && !shut) {
      ...
      if (this.job === null && this.rnd() < 0.55) {
        this.job = this.pick(this.tradesOpen()); this.yearsInJob = 0;
        this.note("finds work " + this.job, "milestone");
      } else if (this.job && this.rnd() < 0.035) {
        const was = this.job;
        const open = this.tradesOpen().filter((t) => t !== was);
        this.job = open.length ? this.pick(open) : was;
        this.yearsInJob = 0;
    ...
    if (age >= 20 && this.rnd() < (0.006 + Math.max(0, age - 35) * 0.0018) *
                                  (2 - this.health)) {
      const [text, years, risk, does, clears] = this.pick(AILMENTS);
  ...
  takeHousehold(what, age) {
    ...
    if (kind === "meet") {
      ...
      const [text, years, does, clears] = SPELLS[what[1]];
      this.circumstances.push({ text: text.replace("%s", what[2]),
        until: age + years, risk: 0, kind: "spell", does, thinks: [], clears });
    } else if (kind === "leaves") {
  ...
  takeEffect(key) {
    ...
    if (key === "school") {
      ...
      this.hadJob = true;
      this.job = this.pick(this.tradesOpen());
  ...
  tradesOpen() {
    return this.average >= 2.45 ? TRADES : TRADES.filter((t) => !SCHOOLED.has(t));
  }
  ...
  birthday(age) {
    ...
    for (const c of this.circumstances) {
      if (c.until <= age && c.clears) this.note(c.clears, "kept");
    }
